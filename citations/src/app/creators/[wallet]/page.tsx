...
export default async function CreatorPage({ params }: Props) {
        ...
        <main className="shell receipt-page" id="main">
          ...
          {mySources.length > 0 && (
            <section className="receipt-context profile-section">
              <div className="panel-heading">
                <p className="eyebrow">your registered work</p>
                <h3>Sources attached to this wallet</h3>
              </div>
              <div className="source-list source-catalog-list">
                {mySources.map((source) => (
                  <SourceCard
                    actionLabel="Verify or view"
                    key={source.id}
                    source={source}
                  />
                ))}
          ...
          <section className="receipt-context profile-section">
            <div className="panel-heading">
              ...
              <h3>Move this wallet from registered to cited</h3>
            </div>
            <div className="hero-cta">
              <Link className="cta-primary" href="/register">
                Register another
              </Link>
              {firstRegisteredSource ? (
                <Link
                  className="cta-secondary"
                  href={`/sources/${firstRegisteredSource.id}#verify`}
                >
                  Verify ownership
                </Link>
              ) : (
                <Link className="cta-secondary" href="/sources">
                  Browse sources
                </Link>
              )}
  ...
  const earningsByDay = Array.from(
    ...
    .slice(-14);
  const maxDaily = Math.max(1, ...earningsByDay.map(([, amount]) => amount));
      ...
      <main className="shell receipt-page" id="main">
        ...
        <section className="receipt-proof">
          ...
          <div className="proof-copy">
            ...
            <p className="hero-text">
              This is your public earnings page. Every amount links to the
              payment that created it, and you can withdraw any time.
            </p>
          </div>
        </section>
        ...
        <section className="metrics-band profile-metrics">
          ...
          <div className="metric">
            ...
            <strong>
              {claimable === null ? (
                <span className="value-with-caption">
                  -<small>on-chain balance unavailable</small>
                </span>
              ) : (
                formatDollars(Number(claimable))
              )}
            </strong>
          </div>
        ...
        <section className="evidence-grid">
          ...
          <div className="evidence-row">
            ...
            <strong>
              <CopyWallet address={creator.wallet} />
            </strong>
          </div>
          <div className="evidence-row">
        ...
        <section className="receipt-context profile-section">
          <div className="panel-heading">
            <p className="eyebrow">next actions</p>
            <h3>Keep the source loop moving</h3>
          </div>
          <div className="hero-cta">
            <Link className="cta-primary" href="/register">
              Register another
            ...
            <div className="daily-bars">
              {earningsByDay.map(([day, amount]) => (
                <div className="daily-bar" key={day}>
                  ...
                  <strong
                    style={{
                      height: `${Math.max(8, (amount / maxDaily) * 96)}px`,
                    }}
                  />
