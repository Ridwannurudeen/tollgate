...
function projectValue(
  ...
  if (Array.isArray(value)) {
    return value.map((item) => {
      ...
      return projected === OMIT ? "[redacted-host-path]" : projected;
    });
  }
...
export function projectPublicData<T>(value: T): T {
  ...
  return (projected === OMIT ? "[redacted-host-path]" : projected) as T;
}

export function publicQueryRecord(query: QueryRecord): QueryRecord {
  return projectPublicData(query);
}

export function publicPaymentReceipt(receipt: PaymentReceipt): PaymentReceipt {
  return projectPublicData(receipt);
}

export function publicLedger(ledger: Ledger): Ledger {
  return projectPublicData(ledger);
}

export function publicSettlementResult(
  result: SettlementResult,
): SettlementResult {
  return projectPublicData(result);
}
