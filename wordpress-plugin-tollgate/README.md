...
The plugin never signs transactions and never stores private keys. It gates selected posts in WordPress, detects AI crawler user agents with a real HTTP 402 response, and checks existing receipts through Tollgate's hosted `/api/wordpress/*` API. The public WordPress callback does not initiate FeeRouter settlement without reader payment authorization.

## Install

1. Copy `tollgate.php` into `wp-content/plugins/tollgate/tollgate.php`.
2. Activate "Tollgate Pay-Per-Read" in WordPress.
3. Ask the Tollgate operator to register the site URL, payout wallet, and price with an operator-issued registration capability, then paste the returned site API key under Settings -> Tollgate.
...
In Docker mode, the check starts `wp-env`, activates the mounted plugin through WP-CLI, configures a local mock Tollgate API, logs into wp-admin, renders Settings -> Tollgate, opens the seeded post editor, saves the Tollgate meta box, verifies the saved post meta, and confirms that the plugin REST pay route returns HTTP 402 without calling the hosted pay API. It does not call the live hosted Tollgate API. Run `node tests/wp-env-smoke.mjs` directly when you specifically want to require the Docker runtime.
...
3. Have the Tollgate operator register the local site with the hosted API using the server-side registration capability, then paste the returned site API key under Settings -> Tollgate. Do not commit either credential.
4. Open a gated post and verify that an existing matching receipt unlocks it. Public reader settlement remains disabled until an end-to-end reader payment authorization is implemented.
...
The manual server seeds a gated post at `http://127.0.0.1:9417/hello-tollgate/` and points the plugin at `http://127.0.0.1:3091` for the hosted Tollgate API. In WordPress Playground, the query-form singular route `http://127.0.0.1:9417/?p=4` is the reliable URL for testing the AI-agent HTTP 402 branch. Start the citations app separately to test existing-receipt status checks.
