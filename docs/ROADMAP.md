...
- **One hot wallet is the payer for every settlement — unresolved, and worse than previously documented.** `createFeeRouterSigner` still signs every `pay()` from a single account. What the last revision missed: `FACILITATOR_PRIVATE_KEY` and `LEPTONWEB_FEE_ROUTER_PRIVATE_KEY` resolve to the *same address*, so the x402 facilitator settles reader payments from the account the FeeRouter is allocating nonces for. That collision took every paid query down from 2026-07-18 to 2026-08-04 — readers were charged and handed a 502 with no receipt. `2ad6b9c` makes the collision recover instead of fail, which means the coupling is now *quiet* rather than gone. Splitting the keys is the actual fix and it is the top item below.
...
- **Splits are still a local JSON registry — unresolved in the product, solved only in the SDK.** `ensureCreatorSplit` still reads and writes a single `data/fee-router-splits.json` under `splitRegistryLock`. `pay-per-piece` ships a pluggable `split-registry.ts` with a file store, so an integrator gets tenant isolation; Citations and Aperture do not use it. A second tenant inside our own deployment would still collide.
...
1. **Split the facilitator and FeeRouter keys.** The one unresolved constraint that has already caused a production incident. Everything below is less urgent than this, and `2ad6b9c` having silenced the symptom makes it *easier* to forget, not safer to defer.
2. **Per-tenant split registry in the product.** Adopt the SDK's pluggable store in Citations/Aperture so the flat `fee-router-splits.json` stops being the single writable choke point. Prerequisite for a second tenant of any kind.
3. **Parallelise `routeCitationPayments`.** Now a bounded-concurrency wrapper around an existing primitive rather than the risky redesign it once was. Latency, not correctness.
4. **One real external integrator.** The published SDK has never been used by someone outside the founding team without hand-holding. Until it has, we are guessing about onboarding friction.
5. **The `auth_request` gate as a config-driven recipe** — Caddy and a Cloudflare Worker variant alongside nginx, so "add per-piece payment to existing software" stops being VPS-specific tribal knowledge.
6. **ORCID author verification.** Still unbuilt; no `orcid` anywhere in `citations/src`. For academic content the creator can't meta-tag a third-party PDF or edit a journal's DNS. "Sign in with ORCID" (OAuth, `/authenticate`), then match the registered DOI against `GET api.orcid.org/v3.0/{iD}/works`. Needs a DOI field on source registration. Free API, non-commercial terms.
