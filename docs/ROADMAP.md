...
- **Concurrent citation payouts** (`e58069d`) — `routeCitationPayments` settled each creator one round-trip after another; they now run together, guarded by `allSettled` so a failure cannot return while sibling payouts are still unconfirmed. Correctness verified in production. Worth recording honestly: three canary runs showed no end-to-end improvement, because payouts are a small slice of a ~50s request dominated by answer generation. The serialisation is gone; the latency win is not yet visible and will not be until either citation counts or answer speed change.
- **The payment gate as a portable recipe** (`docs/RECIPE-PAYMENT-GATE.md`) — the contract behind the nginx `auth_request` gate written down provider-agnostically, with a unit-tested Cloudflare Worker and a Caddyfile. The Caddyfile is explicitly labelled unvalidated: the host runs nginx and has no caddy binary, so it was written from documented behaviour, not observed behaviour.
