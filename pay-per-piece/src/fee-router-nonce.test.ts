...
describe("withReservedNonce", () => {
  beforeEach(() => {
    resetFeeRouterNonceStateForTests();
    delete process.env.TOLLGATE_FEE_ROUTER_NONCE_CONCURRENCY;
  });

  it("reserves increasing nonces for concurrent calls on one account", async () => {
  ...
  it("bounds concurrent submission tasks", async () => {
    process.env.TOLLGATE_FEE_ROUTER_NONCE_CONCURRENCY = "2";
